# @async-fn/vitest — LLM Usage

> Late-resolve async mocks for Vitest. `asyncFn()` returns a `vi.fn()`-compatible mock whose returned promise can be resolved or rejected from the test body — *after* the system under test has called the mock. This lets tests read chronologically.
...
Peer dependency: `vitest >= 1.0.0`. ESM-only (`"type": "module"`).
...
The returned mock is a `vitest.Mock<TFn>` extended with three methods:
...
### Use Vitest's native assertions
...
Types ship at `./src/asyncFnForVitest.d.ts`. `asyncFn<TFn>()` returns `AsyncFnMock<TFn>` — `vitest.Mock<TFn>` intersected with `{ resolve, reject, resolveSpecific }`. Pass the function signature you want to mock as the type parameter:
