...
**asyncFn** for bun provides additional methods to the `mock` from `bun:test` to introduce "**late resolve**" for the promises returned.
...
### The other stuff `mock` does

```javascript
import asyncFn from '@async-fn/bun';

it('does what bun:test mock does', () => {
  const mockFunction = asyncFn();
...
asyncFn is also available for [jest](https://www.npmjs.com/package/@async-fn/jest), [vitest](https://www.npmjs.com/package/@async-fn/vitest) and [sinon](https://www.npmjs.com/package/@async-fn/sinon).
